...
expect(document.querySelector('.application>article>h4').textContent).to.equal(
    'john@abv.bg',
    'Email is invalid.'
);
expect(
    document.querySelectorAll('.application>article>p')[0].textContent
).to.equal('Event:World Wetlands Day', 'Event is invalid.');
expect(
    document.querySelectorAll('.application>article>p')[1].textContent
).to.equal('Location:United Nations', 'Location is invalid');
